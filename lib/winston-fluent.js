...
var Fluent = exports.Fluent = function (options) {
  ...
  var self = this;
  self.fluentSender.on('error', function (err){
    console.log('winston-fluent:: Socket disconnected. ' + err);
    if (err == 'ECONNRESET'){
      console.log('winston-fluent:: Caught ECONNRESET. ');
      console.log('winston-fluent:: Trying to reconnect.');
      self.fluentSender._connect(function(){
        console.log('winston-fluent:: Reconnected.');
      });
    }
  });
};
